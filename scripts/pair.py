...
def choose_iface(prompt: str) -> str:
    ...
    for i, name in enumerate(ifaces):
        driver = "unknown"
        drv = Path(f"/sys/class/net/{name}/device/driver")
        if drv.exists():
...
def init_wpa(conf: Path, iface: str, log: Path) -> "pexpect.spawn":
    cmd = f"{wpa_bin('wpa_supplicant')} -Dnl80211 -i{iface} -c{conf}"
    child = pexpect.spawn(cmd, env=ld_env(), encoding="utf-8", timeout=10)
    child.logfile = open(log, "a", encoding="utf-8")
    try:
        child.expect("Successfully initialized wpa_supplicant", timeout=8)
    except pexpect.ExceptionPexpect:
        die("wpa_supplicant failed to start. See logs/pair-*.log (NetworkManager still owning the iface?)")
    return child
...
def pair_wiiu(iface: str, log: Path, pin: str | None = None, noninteractive: bool = False) -> tuple[str, str, str]:
    ...
    conf.write_text(src.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
    ...
    cli = pexpect.spawn(
        f"{wpa_bin('wpa_cli')} -p/var/run/wpa_supplicant_drc",
        env=ld_env(),
        encoding="utf-8",
        timeout=20,
    )
    ...
    try:
        cli.expect(["> ", "Interactive mode"], timeout=15)
    except pexpect.ExceptionPexpect:
        die("wpa_cli could not attach. See the pair log.")
